...
function updateCallBackground(status=null) {
  const isDarkMode = window.matchMedia('(prefers-color-scheme: dark)')
  ...
  chrome.tabs.executeScript(null, {
    ...
    code: "document.getElementsByClassName('crqnQb')[0].style.backgroundColor = '"+bgColor+"';"
  });
  ...
  if (request.action === 'leaving') {
    ...
    if (sender.tab) {
      ...
      if (request.muted == 'unknown') {
        ...
        chrome.browserAction.setBadgeText({text: ''})
        chrome.browserAction.setTitle({title: 'Toggle Mute in Meeting'})
        updateCallBackground({muted: request.muted})
